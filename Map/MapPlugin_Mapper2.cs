    ...
    public partial class MapPlugin
    {
        ...
        public static void SetPixel(Bitmap bit, int x, int y, System.Drawing.Color color, bool log)   // To prevent out of memory errors
        {
            ...
            catch (Exception) // "The operation failed" - http://msdn.microsoft.com/en-us/library/system.drawing.bitmap.setpixel(v=vs.110).aspx
            {
                ...
                if (log)
                {
                    ProgramLog.BareLog(ProgramLog.Plugin, "<map> WARNING: could not draw certain pixel (" + x + "," + y + ").");
                }
        ...
        public void mapWorld2()
        {
            ...
            if (highlight)
            {
                ...
                if (state != 2)
                {
                    ...
                    catch (KeyNotFoundException e)
                    {
                        ...
                        ProgramLog.BareLog(ProgramLog.Plugin, e.StackTrace.ToString() + ", with key " + (FADE_START_INDEX + (y + y1)).ToString());
            ...
            catch (OutOfMemoryException e)
            {
                ...
                ProgramLog.BareLog(ProgramLog.Error, "<map> ERROR: not enough memory to start mapping threads.");
                ProgramLog.BareLog(ProgramLog.Plugin, e.StackTrace.ToString());
            ...
            {
                ProgramLog.BareLog(ProgramLog.Error, "<map> ERROR: could not create final Bitmap object.");
                ProgramLog.BareLog(ProgramLog.Plugin, e.StackTrace.ToString());
                stopwatch.Stop();
                isMapping = false;
                return;
            ...
            if (pixelfailureflag)
            {
                ProgramLog.BareLog(ProgramLog.Plugin, "<map> WARNING: pixel fail write on hlchests.");
                pixelfailureflag = false;
            }
        ...
        private void maprenderloop(int i, Bitmap bmp, int piece, int ymin, int ymax)
        {
            ...
            {
                ProgramLog.BareLog(ProgramLog.Error, "<map> ERROR: Problem with pixel lookup at (x,y): (" + i + "," + j + "). Key: " + e.Data.Keys.ToString());
                ProgramLog.BareLog(ProgramLog.Plugin, e.StackTrace.ToString());
            ...
            if (pixelfailureflag)
            {
                ProgramLog.BareLog(ProgramLog.Plugin, "<map> WARNING: Could not draw certain pixel at row (" + i + ",y).");
                pixelfailureflag = false;
            }
